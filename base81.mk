...
alpine-base81:
	docker build --platform=linux/amd64 -f "base/8.1/alpine.dockerfile" -t $(NAME)-$(VERSION) .
	@if [ "${PUSH}" == 'true' ]; then \
...
alpine-base-xdebug81:
	docker build --platform=linux/amd64 -f "base-xdebug/8.1/alpine.dockerfile" -t $(NAME)-$(VERSION) .
	@if [ "${PUSH}" == 'true' ]; then \
	    docker push $(NAME)-$(VERSION) ; \
	fi
